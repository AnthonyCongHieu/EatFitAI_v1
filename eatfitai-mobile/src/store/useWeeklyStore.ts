...
export const useWeeklyStore = create<WeeklyStore>((set, get) => ({
    ...
    submitCheckIn: async (request: CheckInRequest) => {
        try {
            ...
            const message = error?.response?.data?.message || error?.message || 'Không thể check-in';
    ...
    fetchHistory: async (page = 1) => {
        try {
            ...
            const message = error?.response?.data?.message || error?.message || 'Không thể tải lịch sử';
            set({ error: message, isLoading: false });
        }
    },
    ...
    fetchSummary: async () => {
        try {
            ...
            const message = error?.response?.data?.message || error?.message || 'Không thể tải thống kê';
            set({ error: message, isLoading: false });
        }
    },
