  ...
  const stopRecording = useCallback(async (): Promise<string | null> => {
    ...
    try {
      ...
      if (uri) {
        ...
        // send audio to Google Speech-to-Text, Azure, or Whisper API

        // Simulated STT result for demo purposes
        // In production, replace with actual STT API call
        const simulatedText = await simulateSTT(uri);

        if (simulatedText) {
          await processText(simulatedText);
        }
...
/**
 * Simulated STT for demo purposes
 * In production, replace with actual STT service
 */
async function simulateSTT(audioUri: string): Promise<string> {
  // This is a placeholder - in real app, you would:
  // 1. Upload audio to your backend
  // 2. Backend calls Google/Azure/Whisper STT
  // 3. Return transcribed text

  // For demo, return empty string (user will type manually)
