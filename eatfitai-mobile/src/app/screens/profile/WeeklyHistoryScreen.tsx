...
const WeeklyHistoryScreen = (): JSX.Element => {
        ...
        >
          ...
          <View style={styles.summaryRow}>
            <View style={styles.summaryItem}>
              <ThemedText style={styles.summaryValue}>{summary.totalWeeks}</ThemedText>
            ...
            <View style={styles.summaryItem}>
              <ThemedText style={styles.summaryValue}>{summary.streak}</ThemedText>
  ...
  const renderCheckInItem = ({
    item,
    index,
  }: {
    item: WeeklyCheckInData;
    index: number;
  }) => {
            ...
            <View style={styles.stat}>
              ...
              {item.weightChange != null && (
                <ThemedText variant="caption" style={{ color: weightChangeColor }}>
                  {item.weightChange > 0 ? '↑' : item.weightChange < 0 ? '↓' : '→'}{' '}
                  {Math.abs(item.weightChange).toFixed(1)} kg
                </ThemedText>
              )}
            </View>
  ...
  return (
    <Screen scroll={false} style={styles.container}>
      <ScreenHeader title="Lịch Sử Check-in" subtitle="Theo dõi tiến độ hàng tuần" />
