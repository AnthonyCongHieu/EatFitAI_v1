...
const NutritionSuggestScreen = (): JSX.Element => {
  ...
  const onSuggest = useCallback(async () => {
    ...
    try {
      ...
      handleApiErrorWithCustomMessage(e, {
        server_error: {
          text1: 'L?i',
          text2: 'M?y ch? ?ang g?p s? c?. Vui l?ng th? l?i.',
        },
        network_error: { text1: 'Kh?ng c? k?t n?i', text2: 'Ki?m tra m?ng v? th? l?i' },
        validation: { text1: 'D? li?u ch?a h?p l?', text2: 'Ki?m tra th?ng tin ??u v?o' },
        unknown: {
          text1: 'Kh?ng g?i ? ???c. Ki?m tra d? li?u.',
    ...
    <Screen contentContainerStyle={{ padding: 16 }}>
      <AppCard padding="lg" shadow="md">
        ...
        <View style={{ flexDirection: 'row', gap: 12, marginBottom: 12 }}>
          <Button
            title={sex === 'male' ? 'Nam ✓' : 'Nam'}
            onPress={() => setSex('male')}
            variant={sex === 'male' ? 'primary' : 'secondary'}
          />
          <Button
            title={sex === 'female' ? 'Nữ ✓' : 'Nữ'}
        ...
        />
        <Input
          label="% Mỡ cơ thể (Tùy chọn)"
          value={bodyFat}
          onChangeText={setBodyFat}
          keyboardType="numeric"
        />
        <Input
          label="Hoạt động (1.2,1.38,1.55,1.73,1.9)"
          value={activityLevel}
        ...
        <View style={{ flexDirection: 'row', gap: 12, marginBottom: 12 }}>
          <Button
            title={goal === 'cut' ? 'Giảm mỡ ✓' : 'Giảm mỡ'}
            onPress={() => setGoal('cut')}
            variant={goal === 'cut' ? 'primary' : 'secondary'}
          />
          <Button
            title={goal === 'maintain' ? 'Giữ ✓' : 'Giữ'}
            onPress={() => setGoal('maintain')}
            variant={goal === 'maintain' ? 'primary' : 'secondary'}
          />
          <Button
            title={goal === 'bulk' ? 'Tăng cơ ✓' : 'Tăng cơ'}
        ...
        <View style={{ gap: 8 }}>
          ...
          {result && (
            <View style={{ marginTop: 12 }}>
              ...
              <View style={{ height: 8 }} />
              <Button
                title={applying ? 'Đang áp dụng...' : 'Áp dụng'}
                onPress={onApply}
                loading={applying}
                variant="secondary"
              />
