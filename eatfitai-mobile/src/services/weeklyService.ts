...
class WeeklyService {
  ...
  async getCurrentWeek(): Promise<CurrentWeekResponse> {
    const { data } = await apiClient.get<CurrentWeekResponse>(
      '/api/weeklycheckin/current',
    );
    return data;
  }
  ...
  async getHistory(page = 1, pageSize = 10): Promise<WeeklyHistoryResponse> {
    const { data } = await apiClient.get<WeeklyHistoryResponse>(
      '/api/weeklycheckin/history',
      {
