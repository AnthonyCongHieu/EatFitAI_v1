...
export const AIResultEditModal: React.FC<AIResultEditModalProps> = ({
    ...
    const calculatedCalories = ((parseFloat(calories) || 0) * (parseFloat(grams) || 100)) / 100;
            ...
            >
                <View style={[styles.container, { backgroundColor: theme.colors.background }]}>
                    <View style={styles.header}>
                        <ThemedText variant="h3" weight="700">{item.label}</ThemedText>
                        <Pressable onPress={onClose} hitSlop={10}>
                            <Icon name="close" size="md" color="textSecondary" />
                        </Pressable>
                    </View>

                    <ThemedText variant="caption" color="textSecondary" style={{ marginBottom: theme.spacing.lg }}>
                        Độ tin cậy: {Math.round(item.confidence * 100)}%
                    </ThemedText>

                    {/* Amount Input */}
                    <View style={styles.inputRow}>
                        <ThemedText variant="body" weight="600" style={styles.inputLabel}>Khối lượng (g)</ThemedText>
                        <TextInput
                    ...
                    <View style={styles.nutritionGrid}>
                        <View style={styles.nutritionItem}>
                            ...
                            <TextInput
                                style={[styles.smallInput, { backgroundColor: theme.colors.card, color: theme.colors.text, borderColor: theme.colors.border }]}
                                value={calories}
                        ...
                        <View style={styles.nutritionItem}>
                            <ThemedText variant="caption" color="textSecondary">Protein (g)</ThemedText>
                            <TextInput
                                style={[styles.smallInput, { backgroundColor: theme.colors.card, color: theme.colors.text, borderColor: theme.colors.border }]}
                                value={protein}
                        ...
                        <View style={styles.nutritionItem}>
                            <ThemedText variant="caption" color="textSecondary">Carbs (g)</ThemedText>
                            <TextInput
                                style={[styles.smallInput, { backgroundColor: theme.colors.card, color: theme.colors.text, borderColor: theme.colors.border }]}
                                value={carbs}
                        ...
                        <View style={styles.nutritionItem}>
                            <ThemedText variant="caption" color="textSecondary">Fat (g)</ThemedText>
                            <TextInput
                                style={[styles.smallInput, { backgroundColor: theme.colors.card, color: theme.colors.text, borderColor: theme.colors.border }]}
                                value={fat}
                                onChangeText={setFat}
                    ...
                    <View style={[styles.totalRow, { backgroundColor: theme.colors.primaryLight }]}>
                        <ThemedText variant="body" weight="600">Tổng calories</ThemedText>
