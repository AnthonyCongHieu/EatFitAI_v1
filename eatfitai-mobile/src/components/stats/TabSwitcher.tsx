...
export const TabSwitcher: React.FC<TabSwitcherProps> = ({
    ...
    const indicatorPosition = useSharedValue(TABS.findIndex(t => t.key === activeTab));

    const handleTabPress = (tab: TabOption, index: number) => {
        Haptics.impactAsync(Haptics.ImpactFeedbackStyle.Light);
        indicatorPosition.value = withSpring(index, { damping: 15 });
        onTabChange(tab);
    };

    const indicatorStyle = useAnimatedStyle(() => ({
        transform: [{ translateX: indicatorPosition.value * (100 / TABS.length) + '%' as any }],
    }));
