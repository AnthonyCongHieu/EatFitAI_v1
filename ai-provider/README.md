...
If the existing pool is already rate-limited, set `GEMINI_EXHAUSTED_PROJECT_IDS` so the service skips those project IDs immediately after boot instead of spending the first few requests rediscovering the same `429`s.
