...
DATASET_DIR = KAGGLE_WORKING / os.environ.get("EATFITAI_YOLO_DATASET_DIR_NAME", "eatfitai_clean_v4_class_expansion")
RUN_PROJECT = KAGGLE_WORKING / "runs" / "food-detection"
RUN_NAME = os.environ.get("EATFITAI_YOLO_RUN_NAME", "yolo11m-eatfitai-clean-v4-class-expansion")
...
MOUNTED_DATASET_DIR_NAMES = (
    "eatfitai_dataset_v2_clean_v4_class_expansion_candidate",
)
...
def has_yolo_split_dirs(dataset_dir: Path) -> bool:
    return (
        ...
        and (dataset_dir / "images" / "train").is_dir()
        and (dataset_dir / "labels" / "train").is_dir()
    )


def find_mounted_dataset_dir(input_root: Path = KAGGLE_INPUT) -> Path | None:
    for name in MOUNTED_DATASET_DIR_NAMES:
        for candidate in sorted(input_root.glob(f"**/{name}")):
            if candidate.is_dir() and has_yolo_split_dirs(candidate):
                return candidate
    for data_yaml in sorted(input_root.glob("**/data.yaml")):
        candidate = data_yaml.parent
        if candidate.name in MOUNTED_DATASET_DIR_NAMES and has_yolo_split_dirs(candidate):
            return candidate
    return None


def link_dataset_subdirs(source_dir: Path, target_dir: Path) -> None:
    for top_name in ("images", "labels"):
        source_top = source_dir / top_name
        target_top = target_dir / top_name
        target_top.mkdir(parents=True, exist_ok=True)
        for split_dir in sorted(path for path in source_top.iterdir() if path.is_dir()):
            target_split = target_top / split_dir.name
            if target_split.exists() or target_split.is_symlink():
                if target_split.is_dir() and not target_split.is_symlink():
                    shutil.rmtree(target_split)
                else:
                    target_split.unlink()
            os.symlink(split_dir, target_split, target_is_directory=True)


def prepare_mounted_dataset_view(source_dir: Path, target_dir: Path) -> Path:
    if target_dir.exists():
        shutil.rmtree(target_dir)
    target_dir.mkdir(parents=True, exist_ok=True)
    link_dataset_subdirs(source_dir, target_dir)
    original = source_dir / "data.yaml"
    lines = original.read_text(encoding="utf-8").splitlines()
    rebased_lines: list[str] = []
    replaced_path = False
    for line in lines:
        if line.lstrip().startswith("path:"):
            rebased_lines.append(f"path: {target_dir.as_posix()}")
            replaced_path = True
        else:
            rebased_lines.append(line)
    if not replaced_path:
        rebased_lines.insert(0, f"path: {target_dir.as_posix()}")
...
def find_extracted_dataset_dir(search_root: Path, working_dir: Path) -> Path:
    preferred = (
        search_root / "eatfitai_dataset_v2_clean_v4_class_expansion_candidate" / "data.yaml",
...
def extract_dataset() -> Path:
    mounted_dataset = find_mounted_dataset_dir()
    if mounted_dataset is not None:
        print(f"Using mounted clean V4 dataset directory: {mounted_dataset}", flush=True)
        return prepare_mounted_dataset_view(mounted_dataset, DATASET_EXTRACT_DIR / DATASET_DIR.name)
    ...
    if not skip_smoke:
        if resume_checkpoint and SKIP_SMOKE_ON_RESUME:
            ...
            smoke.train(
                ...
                name="smoke-yolo11s-clean-v4-class-expansion",
                exist_ok=True,
                cache=False,
                amp=True,
            )
