...
REPORT_DIR = Path("/kaggle/working/_eatfitai_v4_targeted_vietnamese_audit_reports")
REPORTS_ZIP = Path("/kaggle/working/eatfitai_v4_targeted_vietnamese_audit_reports.zip")
SOURCE_MANIFEST = os.environ.get(
    "EATFITAI_V4_TARGETED_VIETNAMESE_SOURCE_MANIFEST",
    "clean_v4_targeted_vietnamese_source_candidates_2026-05-14.csv",
)
MAX_RUNTIME_SECONDS = int(os.environ.get("EATFITAI_V4_TARGETED_AUDIT_MAX_SECONDS", "18000"))
...
def write_reports(
    ...
    summary["kernel_kind"] = "v4_targeted_vietnamese_source_audit"
...
def main() -> int:
    ...
    for index, row in enumerate(manifest, start=1):
        ...
        print(f"V4_TARGETED_VIETNAMESE_AUDIT_PROGRESS {index}/{len(manifest)} {dataset_ref}", flush=True)
        try:
