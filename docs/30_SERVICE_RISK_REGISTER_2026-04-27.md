...
| AI provider -> packaged model asset | model weights bundled with service image | no Supabase model bucket runtime dependency | stale artifact or generic fallback if deploy/build fails |
...
| SR-08 | Supabase | Service role key should be limited to backend dependency checks and controlled data flows; AI provider model download has been removed. | broad data/storage access if leaked. | Medium. | High | Backend uses `Supabase__ServiceRoleKey`; current `render.yaml` no longer declares AI provider `SUPABASE_SERVICE_KEY`. Live Render env may still contain stale AI provider Supabase vars until cleaned. | Rotate after exposure; remove stale AI provider Supabase env; keep model assets packaged/R2-managed, not Supabase Storage. | Backend/AI | In progress |
